(function() {
    ...
    var B = d.append("g").attr("class", "links"),
        f = d.append("g").attr("class", "userstories"),
    ...
    d3.json("/defect", function(X, Y) {
        ...
        j = d3.map();
        T.get("userstories").forEach(function(aa) {
    ...
    function O() {
        ...
        i = Math.floor(c / T.get("userstories").length);
        y = Math.floor(T.get("userstories").length * i / 2);
        T.get("userstories").forEach(function(af, ae) {
            af.x = U / -2;
            af.y = ae * i - y
        });
        var ad = 180 + J,
            Z = 360 - J,
            ac = (Z - ad) / (T.get("defects").length - 1);
        ...
        H = [];
        var ab, Y, aa, X = h / 2 - R;
        T.get("userstories").forEach(function(ae) {
    ...
    function M() {
        ...
        D(T.get("userstories"));
        b(d3.merge([T.get("defects"), T.get("testcases")]));
        C([]);
        m(P);
    ...
    function C(Z) {
        ...
        if (ab && ab.type === "userstory") {
            ...
            aa.append("text").attr("fill", N).attr("text-anchor", "middle").attr("y", (o + t) * -1).text(function(ae) {
                return "userstory " + ae.userstory
            })
        } else {
            if (ab && ab.type === "defect") {
                Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("defect")
            } else {
        ...
        ac.exit().remove();
        var X = d.selectAll(".all-userstories").data(Z);
        X.enter().append("text").attr("text-anchor", "start").attr("x", a / -2 + t).attr("y", c / 2 - t).text("all userstories").attr("class", "all-userstories").on("click", O);
        X.exit().remove()
    }

    function D(Y) {
        var Y = f.selectAll(".userstory").data(Y, u);
        var X = Y.enter().append("g").attr("class", "userstory").on("mouseover", g).on("mouseout", n).on("click", G);
    ...
    function z() {
        ...
        E.selectAll("circle").attr("fill", function(X) {
            ...
            if (X === L.node) {
                ...
                if (X.type === "defect" || X.type === "testcase") {
                    return 1.5
                }
            }
            return null
